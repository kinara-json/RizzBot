...
const buy = async (m, args, db) => {
	if (args[0] === 'limit') {
		...
		if (db.users[m.sender].money >= count * 2000) {
			db.users[m.sender].limit += count * 1
			db.users[m.sender].money -= count * 2000
			m.reply(`Berhasil Membeli Limit Sebanyak ${args[1] * 1} dengan harga ${args[1] * 2000}`);
	...
	} else m.reply(`Harga Limit : Jumlah x 2000\n• 1 limit = 2000\n• 2 limit = 4000\n\nExample : .buy limit 3`);
}

// Fungsi untuk mengurangi limit sesuai jumlah yang diinginkan
const setLimit = (m, db, amount = 1) => {
  if (!db.users[m.sender]) db.users[m.sender] = { limit: 0 }
  db.users[m.sender].limit -= amount
  return db.users[m.sender].limit
}
